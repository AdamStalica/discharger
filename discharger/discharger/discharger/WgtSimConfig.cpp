...
json select = {
	...
	prepareTimeLine(const json & data) {

	std::vector<int> durationBetweenLogs(data.size() + 1);
	std::vector<int> durationBetweenLogs2(data.size() + 1);
	...
	std::transform(data.begin() + 1, data.end(), durationBetweenLogs.begin() + 2, [&lastLogTime, &timeStrToMs](const json & log)->int {

		int currentLogTime = timeStrToMs(log["curr_race_time"].get<std::string>());
